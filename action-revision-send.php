	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' ) {  // Pokud se zmáčkne tlačítko POST, udělej funkci
		//if ( isset($_POST['otazka-1']) ) {
			...
			//if (empty($_POST['image-1'])){
				//echo "Obrázek 1 byl editován<br>";
			...
			if (isset_file('image-2') && $error == false ) {
			//if (isset($_FILES['image-2']) && $error == false) {
			//if (empty($_POST['image-2'])) {
				//echo "Obrázek 2 byl editován<br>";
			...
			if (isset_file('image-3') && $error == false ) {
			//if (isset($_FILES['image-3']) && $error == false) {
			//if (empty($_POST['image-3'])) {
				//echo "Obrázek 3 byl editován<br>";
			...
			//echo "Celková ks: $celkemKusu Kč,-<br>Celkem Others: $othersKs<br>";
			...
			if ( $error == false ) {
				...
				if (mysqli_query($conn, $sql)) {
					echo "<div class=\"message-success\">Akce byla úspěšně upravena</div><br>";
