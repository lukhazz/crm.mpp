<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' ) { // confirm that the 'id' value is a valid integer before getting the form data
		...
		if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
				...
				if (!empty($QuestionArray[1])) {
					...
					for ($k=0; $k < count(${$testTemp}); $k++) { // Cyklus projizdejici odpovedi uzivatele 
						...
						for ($l=0; $l < count($QuestionArray); $l++) { 
							//echo "QuestionArray $l - $QuestionArray[$l]<br>";
						...
						//echo "Odpovědí je ".$counterOdpovedi." a uživatel označil ".count(${$testTemp})."<br>";
						if ($counterOdpovedi!=count(${$testTemp})) {
							$questionSuccess = false;
						}
					}
				}		
			...
			$successPercent = str_replace(",",".","$successPercent");
			...
			if ( $error == false ) {
				...
				if (mysqli_query($conn, $sqlOdeslano) ) {
					...
					//echo "chyba SQL dotazu<br>";
					$messageError = "1) Nepodařilo se uložit výsledek testu do databáze.";
				}
		...
		if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) { // query db
			...
			if ( $row[0] == NULL ) {
				if (mysqli_query($conn, $sqlDateStart)) {
					$messageSuccess = "Úspěšně zahájen pokus o splnění testu v čase $dateStart<br>";
				}
				else {
					//echo "chyba SQL dotazu při zapisování startovacího datumu $dateStart<br>";
					$messageError = "Chyba při zahájení pokus o splnění testu v čase $dateStart<br>";
				}
			}

			else {
				$dateStart = $row[0];
				$messageSuccess = "Úspěšně obnoven pokus o splnění testu v čase, zahájený $dateStart<br>";
			}
